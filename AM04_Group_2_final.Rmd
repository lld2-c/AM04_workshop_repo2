...
grid.arrange(pamcenters,graphkmeans_4clusters,ncol = 1)


```

```{r,pam pca,message=FALSE,warning=FALSE,fig.height=4,fig.width=6}
fviz_cluster(k4_pam, data = userdata_train2) + ggtitle("PAM k = 4") + 
  #scale_colour_manual(values = c("darkgreen", "orange", "red","blue")) +
  theme_minimal() +
  theme(text = element_text(size=10),
        plot.title = element_text(size = 12, face = "bold", vjust = 2)) 
...
grid.arrange(hclust_centre,graphkmeans_4clusters,ncol = 1)
...
```{r out of sample check,fig.height=8,fig.width=8}
