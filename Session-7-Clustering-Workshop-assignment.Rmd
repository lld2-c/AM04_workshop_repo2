...
* For the first elbow plot, SSE did not stailise within k = 10, thus we will be looking for points in the middle to ensure that we can have a relatively low SSE while not overfitting the data. In Silhoutte analysis, we will be looking for points with high silhoutte width but still keep in mind the problem of overfitting. Overall, the best possible values located for k are 4 and 6.
...
```{r different k pca, fig.height=8, fig.width=4}
#PCA visualizations
