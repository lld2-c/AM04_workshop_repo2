...
#add your results to the data frame userdatall
#Let's add number of shows watched more than 60 percent for each user
userData6 <- cleaned_BBC_Data %>% group_by(user_id) %>% summarise(n_over_60pct=sum(watched_more_60_percent))
...
The variables that are most correlated are those that we would expect. Firstly, there is a perfect negative correlation between ‘weekend’ and ‘weekday’, as these variables represent the percentage of the user’s watch time that comes on the weekend and during the week respectively. The higher the percentage of watchtime that occurs on the weekend, the lower the watchtime percentage will be during the week (and vice versa) - therefore, these variables have to have a perfectly negative correlation. 
...
There are also fairly strong negative correlations between the different parameters for the time of day (‘Evening’, ‘Day’ & ‘Afternoon’) - this is also expected, as the larger proportion of watch time that occurs in the evening, the less that is likely to be done in the day and afternoon. Most of the other variables exhibit weak correlations. 
...
```
* There are 3638 points in one cluster and 5713 points in the other.
...
* The first cluster (pink) contains viewers that have a shorter total watching time and prefer to watch in the afternoon (daytime). The second cluster (blue) contains viewers that have a longer total watching time and prefer to watch in the evening (night). These clusters have a significant difference in their watching behaviour. According to these two clusters, we can roughly conclude that the active users like to use iPlayer during night time
...
* BBC can use the cluster information to build a recommendation system which can advertise TV series that are likely to be welcomed by the major cluster at different time of day and also on different day of week.
...
summary(model2_kmeans_2clusters)
paste0("Size of clusters:")
model2_kmeans_2clusters$size
...
* For the first elbow plot, SSE did not stailise within k = 10, thus we will be looking for points in the middle to ensure that we can have a relatively low SSE while not overfitting the data. In Silhoutte analysis, we will be looking for points with high silhoutte width but still keep in mind the problem of overfitting. Overall, the best range of values located for k is from 5 to 7.
...
* Some repetition of cluster size can be seen. There are always one cluster with size of about 2300 and one about 1200 in all three models.


```{r different k pca, fig.height=6, fig.width=4}
#PCA visualizations
#your code here
# plots to compare
#I use the fviz_cluster function which is part of the`factoextra` library
p1 <- fviz_cluster(model_kmeans_3clusters, geom = "point", data = userdata_train2) + ggtitle("k = 3")
p2 <- fviz_cluster(model_kmeans_4clusters, geom = "point", data = userdata_train2) + ggtitle("k = 4")
p3 <- fviz_cluster(model_kmeans_5clusters, geom = "point", data = userdata_train2) + ggtitle("k = 5")
...
* In the above PCA plots, we can see that as we increase the number of clusters, there are more and more overlap between the clusters. When k = 3, cluster 2 is almost all covered by cluster 1 and cluster 3, giving it very few explanatory power (as in why it should be a separate cluster). When k = 5, cluster 1, 2, 4 and 5 largely overlap and it will also be difficult to distinguish between the characteristics of the clusters. The middle plot (k = 4) seem most reasonable among the three plots. Although there is still overlap present, the clusters are relatively more "independent" compared to the other two models.

* It has been noticed that some clusters are constantly present in each model. For example, cluster 1 in the top plot is very similar to cluster 4 in the middle plot and cluster 1 in the bottom plot. Cluster 2 in the top plot is similar to cluster 3 in the middle plot and cluster 5 in the bottom plot. 
...
graphkmeans_5clusters <- ggplot(cluster_centers_t, aes(x = variable, y = value))+  
  geom_line(aes(color =cluster,group = cluster), linetype = "dashed",size=1)+ 
  geom_point(size=1,shape=4)+
  geom_hline(yintercept=0)+theme(text = element_text(size=10),
        axis.text.x = element_text(angle=45, hjust=1),)+
...
* From the above plot, we can again sense the repitition of clusters as we increase the value of k. The three clusters in the top plot are similar to cluster 2, 3, 4 in the middle plot and cluster 1, 2, 5 in the bottom plot.
